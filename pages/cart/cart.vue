					...
					<view class="cart-item" :class="{ 'b-b': index !== cartList.length - 1 }">
						...
						<view class="item-right">
							...
							<uni-number-box
								...
								:isMin="item.count === 1"
								:index="index"
								@eventChange="handleNumberChange($event, item.skuId)"
							/>
						</view>
...
import { detail, update, choose, checkCartItem, checkAll, deleteCartItem, deleteCart, updateCartItem } from '@/api/oms/cart.js';
	...
	methods: {
		...
		async loadData() {
			detail().then(response => {
				this.cartItems = [];
				this.totalPrice = 0;
				console.log('========购物车详情========', response.data);
				const { items } = response.data;
				if (items.length > 0) {
					this.cartItems = items; //购物车商品列表
					this.handleChangeCart(); //计算总价
				}
			});
		},
