...
async function loadData() {
    try {
        const advertResponse = await getAdvertList();
        ...
        goodsList.value = goodsResponse.data || [];
    } catch (error) {
...
// 根据需要启用或禁用的平台特定代码
const onNavigationBarSearchInputClicked = async (e: any) => {
    console.log('点击了搜索框');
};

const onNavigationBarButtonTap = (e: any) => {
    const index = e.index;
    if (index === 0) {
        console.log('点击了扫描');
    } else if (index === 1) {
