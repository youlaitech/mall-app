            ...
            <swiper-item class="tab-content" v-for="(tabItem, tabIndex) in navList" :key="tabIndex">
                <scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
                    ...
                    <view v-for="(order, index) in tabItem.orderList" :key="index" class="order-item">
                        ...
                        <view v-if="order.orderItems.length === 1" class="goods-box-single" v-for="(orderItem, itemIndex) in order.orderItems" :key="itemIndex">
                            <image class="goods-img" :src="orderItem.picUrl" mode="aspectFill"></image>
                            <view class="right">
...
import { cancelOrder, deleteOrder, listOrdersWithPage } from '@/api/oms/order.js';
...
const navList = ref([
    { status: 0, text: '全部', loadingType: 'more', orderList: [] },
    { status: 101, text: '待付款', loadingType: 'more', orderList: [] },
    { status: 201, text: '已付款', loadingType: 'more', orderList: [] },
    { status: 301, text: '待发货', loadingType: 'more', orderList: [] },
    { status: 901, text: '已完成', loadingType: 'more', orderList: [] },
]);

const loadData = async () => {
    const navItem = navList.value[tabCurrentIndex.value];
    ...
    setTimeout(async () => {
        ...
        const params = { pageNum: pageNum.value, pageSize: pageSize.value, status: orderStatus };
...
onMounted(() => {
    tabCurrentIndex.value = navList.value.map((item) => item.status).indexOf(parseInt(route.query.status as string));
    loadData();
});
...
const cancelOrderHandler = async (item: { id: number }) => {
    // 取消订单的逻辑
};
 ...
 * @param order
 */
const doPay = (order: { orderSn: string; paymentAmount: number }) => {
    uni.redirectTo({
